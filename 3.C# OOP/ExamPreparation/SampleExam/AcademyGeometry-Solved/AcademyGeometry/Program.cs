...
namespace GeometryAPI
{
    ...
    public class Cylinder : Figure, IVolumeMeasurable, IAreaMeasurable
    {
        ...
        public Cylinder(Vector3D bottomVector, Vector3D topVector, double radius)
            : base(bottomVector, topVector)
        {
            this.Radius = radius;
        }

        public Vector3D BottomVector
        {
            get
            {
        ...
        public double GetVolume()
        {
            ...
            double height = (this.TopVector - this.BottomVector).Magnitude;
        ...
        {
            double height = (this.TopVector - this.BottomVector).Magnitude;
            double sideArea = Math.PI * 2 * this.Radius * height;
            double basesArea = this.Radius * this.Radius * Math.PI * 2;
            return sideArea + basesArea;
        }
    }
    ...
    public class FigureController
    {
        ...
        public virtual void ExecuteFigureCreationCommand(string[] splitFigString)
        {
            switch (splitFigString[0])
            {
                ...
                case "cylinder":
                    {
                        Vector3D bottomVector = Vector3D.Parse(splitFigString[1]);
                        Vector3D topVector = Vector3D.Parse(splitFigString[2]);
                        double radius = double.Parse(splitFigString[3]);
                        currentFigure = new Cylinder(bottomVector, topVector, radius);
                        break;
                    }
            }
